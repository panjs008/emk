...
public class EmkEnquiryDetailController
    ...
    public void datagrid(EmkEnquiryDetailEntity emkEnquiryDetail, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        cq.add();
        this.emkEnquiryDetailService.getDataGridReturn(cq, true);
        Map infoMap = this.systemService.findOneForJdbc("SELECT ifnull(SUM(t.total),0) total,ROUND(SUM(t.total*t.price),2) sumprice FROM emk_enquiry_detail t WHERE t.enquiry_id=?", new Object[]{emkEnquiryDetail.getEnquiryId()});
    ...
    public AjaxJson doDel(EmkEnquiryDetailEntity emkEnquiryDetail, HttpServletRequest request) {
        ...
        emkEnquiryDetail = (EmkEnquiryDetailEntity) this.systemService.getEntity(EmkEnquiryDetailEntity.class, emkEnquiryDetail.getId());
        message = "询盘单明细删除成功";
        try {
            this.emkEnquiryDetailService.delete(emkEnquiryDetail);
    ...
    public AjaxJson doBatchDel(String ids, HttpServletRequest request) {
        ...
        try {
            for (String id : ids.split(",")) {
                EmkEnquiryDetailEntity emkEnquiryDetail = (EmkEnquiryDetailEntity) this.systemService.getEntity(EmkEnquiryDetailEntity.class, id);


                this.emkEnquiryDetailService.delete(emkEnquiryDetail);

                Map infoMap = this.systemService.findOneForJdbc("SELECT ifnull(SUM(t.total),0) total FROM emk_enquiry_detail t WHERE enquiry_id=? and t.color=?", new Object[]{emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getColor()});
                emkEnquiryDetail.setColorTotal(Integer.valueOf(Integer.parseInt(infoMap.get("total").toString())));
                infoMap = this.systemService.findOneForJdbc("SELECT ifnull(SUM(t.total),0) total FROM emk_enquiry_detail t WHERE enquiry_id=? and t.size=?", new Object[]{emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getSize()});
                emkEnquiryDetail.setSizeTotal(Integer.valueOf(Integer.parseInt(infoMap.get("total").toString())));
                this.systemService.executeSql("update emk_enquiry_detail set color_total=? where enquiry_id=? and  color=?", new Object[]{emkEnquiryDetail.getColorTotal(), emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getColor()});
                this.systemService.executeSql("update emk_enquiry_detail set size_total=? where enquiry_id=? and size=?", new Object[]{emkEnquiryDetail.getSizeTotal(), emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getSize()});


                infoMap = this.systemService.findOneForJdbc("SELECT ifnull(SUM(t.total),0) total,ROUND(SUM(t.total*t.price),2) sumprice FROM emk_enquiry_detail t WHERE t.enquiry_id=?", new Object[]{emkEnquiryDetail.getEnquiryId()});
                this.systemService.executeSql("update emk_enquiry set sum_total=?,sum_money=? where id=?", new Object[]{infoMap.get("total"), infoMap.get("sumprice"), emkEnquiryDetail.getEnquiryId()});
    ...
    public AjaxJson doAdd(EmkEnquiryDetailEntity emkEnquiryDetail, HttpServletRequest request) {
        ...
        message = "询盘单明细添加成功";
        try {
            this.emkEnquiryDetailService.save(emkEnquiryDetail);
            Map infoMap = this.systemService.findOneForJdbc("SELECT ifnull(SUM(t.total),0) total FROM emk_enquiry_detail t WHERE enquiry_id=? and t.color=?", new Object[]{emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getColor()});
            emkEnquiryDetail.setColorTotal(Integer.valueOf(Integer.parseInt(infoMap.get("total").toString())));
            infoMap = this.systemService.findOneForJdbc("SELECT ifnull(SUM(t.total),0) total FROM emk_enquiry_detail t WHERE enquiry_id=? and t.size=?", new Object[]{emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getSize()});
            emkEnquiryDetail.setSizeTotal(Integer.valueOf(Integer.parseInt(infoMap.get("total").toString())));
    ...
    public AjaxJson doUpdate(EmkEnquiryDetailEntity emkEnquiryDetail, HttpServletRequest request) {
        ...
        AjaxJson j = new AjaxJson();
        message = "询盘单明细更新成功";
        EmkEnquiryDetailEntity t = (EmkEnquiryDetailEntity) this.emkEnquiryDetailService.get(EmkEnquiryDetailEntity.class, emkEnquiryDetail.getId());
        try {
            ...
            this.systemService.executeSql("update emk_enquiry_detail set color_total=? where enquiry_id=? and  color=?", new Object[]{emkEnquiryDetail.getColorTotal(), emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getColor()});
            this.systemService.executeSql("update emk_enquiry_detail set size_total=? where enquiry_id=? and size=?", new Object[]{emkEnquiryDetail.getSizeTotal(), emkEnquiryDetail.getEnquiryId(), emkEnquiryDetail.getSize()});
            this.emkEnquiryDetailService.saveOrUpdate(t);
    ...
    public ModelAndView goAdd(EmkEnquiryDetailEntity emkEnquiryDetail, HttpServletRequest req) {
        if (StringUtil.isNotEmpty(emkEnquiryDetail.getId())) {
            emkEnquiryDetail = (EmkEnquiryDetailEntity) this.emkEnquiryDetailService.getEntity(EmkEnquiryDetailEntity.class, emkEnquiryDetail.getId());
    ...
    public String exportXls(EmkEnquiryDetailEntity emkEnquiryDetail, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid, ModelMap modelMap) {
        CriteriaQuery cq = new CriteriaQuery(EmkEnquiryDetailEntity.class, dataGrid);
        HqlGenerateUtil.installHql(cq, emkEnquiryDetail, request.getParameterMap());
        List<EmkEnquiryDetailEntity> emkEnquiryDetails = this.emkEnquiryDetailService.getListByCriteriaQuery(cq, Boolean.valueOf(false));
    ...
    public ResponseMessage<List<EmkEnquiryDetailEntity>> list() {
        List<EmkEnquiryDetailEntity> listEmkEnquiryDetails = this.emkEnquiryDetailService.getList(EmkEnquiryDetailEntity.class);
    ...
    public ResponseMessage<?> get(@ApiParam(required = true, name = "id", value = "ID") @PathVariable("id") String id) {
        EmkEnquiryDetailEntity task = (EmkEnquiryDetailEntity) this.emkEnquiryDetailService.get(EmkEnquiryDetailEntity.class, id);
        if (task == null) {
    ...
    public ResponseMessage<?> create(@ApiParam(name = "询盘单明细对象") @RequestBody EmkEnquiryDetailEntity emkEnquiryDetail, UriComponentsBuilder uriBuilder) {
        Set<ConstraintViolation<EmkEnquiryDetailEntity>> failures = this.validator.validate(emkEnquiryDetail, new Class[0]);
        if (!failures.isEmpty()) {
    ...
    public ResponseMessage<?> update(@ApiParam(name = "询盘单明细对象") @RequestBody EmkEnquiryDetailEntity emkEnquiryDetail) {
        ...
        try {
            this.emkEnquiryDetailService.saveOrUpdate(emkEnquiryDetail);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseMessage<?> delete(@ApiParam(name = "id", value = "ID", required = true) @PathVariable("id") String id) {
        ...
        try {
            this.emkEnquiryDetailService.deleteEntityById(EmkEnquiryDetailEntity.class, id);
        } catch (Exception e) {
            e.printStackTrace();
