					...
					<div class="liwrap">
						...
						<div class="liright">
							...
							<div class="histt"><a style="font-size: 12px;"><c:if test="${task.TASK_DEF_KEY_ eq 'khxpTask'}">【${task.workname}】发起工单，工单号：${emkWorkOrderPage.workNo}，询盘单号：${emkWorkOrderPage.askNo},处理意见：${emkWorkOrderPage.askWorkAdvice}</c:if>
								<c:if test="${task.TASK_DEF_KEY_ eq 'sampleTask'}">处理人【${task.workname}】，指派给【${emkWorkOrderPage.sampleCheckUser}】，打样单号：${emkWorkOrderPage.sampleNum},处理意见：${emkWorkOrderPage.sampleAdvice}</c:if>
