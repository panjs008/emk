...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="emkMOutStorageList" checkbox="false" pagination="true" fitColumns="true" title="" actionUrl="emkMOutStorageController.do?datagrid&type=1" idField="id" fit="true" btnCls="bootstrap"  queryMode="group">
      ...
      <t:dgToolBar title="录入" icon="fa fa-plus" operationCode="add" url="emkMOutStorageController.do?goAdd&type=1&winTitle=添加缝制辅料出库申请单" funname="add" height="600" width="1100"></t:dgToolBar>
      <t:dgToolBar title="编辑" icon="fa fa-edit" operationCode="edit" url="emkMOutStorageController.do?goUpdate&winTitle=编辑缝制辅料出库申请单" funname="update" height="600" width="1100"></t:dgToolBar>
      <t:dgToolBar title="查看" icon="fa fa-search" operationCode="look" url="emkMOutStorageController.do?goUpdate&goUpdate&winTitle=查看缝制辅料出库申请单" funname="detail" height="600" width="1100"></t:dgToolBar>
 ...
 function doSubmitV() {
     ...
     if (!rowsData || rowsData.length == 0) {
         tip('请选择需要提交的缝制辅料出库申请单');
         return;
     }
     for ( var i = 0; i < rowsData.length; i++) {
         ids.push(rowsData[i].id);
     }
     $.dialog.confirm('您是否确定提交缝制辅料出库申请单?', function(r) {
         if (r) {
             $.ajax({
 ...
 function goToProcess(id,createBy,createName){
     ...
     $.ajax({
         url : "emkMOutStorageController.do?getCurrentProcess&id="+id,
         type : 'post',
         cache : false,
         data: null,
         success : function(data) {
