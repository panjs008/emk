...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="emkMaterialContractController.do?doSubmit"  tiptype="1" >
	...
	<table id="adviceTb" style="width: 100%;margin-top:2px;margin-bottom: 4px" cellpadding="0" cellspacing="1" class="formtable">
		<c:if test="${orderFinish eq '0' && orderPorcess.taskDefinitionKey eq 'checkTask'}">
			<c:set value="财务" var="userKey" scope="session"></c:set>
			<tr>
				<td align="right">
				...
				<td class="value" colspan="3">
					<input name="isPass" type="radio" datatype="*" <c:if test="${emkMaterialContract.isPass eq '0'}">checked="true"</c:if> value="0">
					是
					<input name="isPass" type="radio" datatype="*"  <c:if test="${emkMaterialContract.isPass eq '1'}">checked="true"</c:if> value="1">
					否
		...
		<c:if test="${orderFinish eq '0' && orderPorcess.taskDefinitionKey eq 'ycghtTask'}">
			<c:set value="财务经理" var="userKey" scope="session"></c:set>
		</c:if>
			<c:if test="${orderFinish eq '0' && orderPorcess.taskDefinitionKey eq 'checkTask' || orderPorcess.taskDefinitionKey eq 'ycghtTask'}">
				<tr id="chooseUser">
