				...
				<li>
					<div class="liwrap">
						...
						<div class="liright">
							...
							<div class="histt"><a style="font-size: 12px;"><c:if test="${task.TASK_DEF_KEY_ eq 'testTask'}">【${task.workname}】发起测试申请单，测试申请单号：${emkTest.cssqdh}</c:if>
								<c:if test="${task.TASK_DEF_KEY_ eq 'checkTask'}">审核人【${emkTest.leader}】,处理意见：${emkTest.leadAdvice}</c:if>
							</a></div>
						</div>
					</div>
				</li>
